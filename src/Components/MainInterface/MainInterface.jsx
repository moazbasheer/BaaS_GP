...
function MainInterface({ logOut }) {
  ...
  useEffect(() => {
    ...
    try {
      privateRequest.get('organization/wallet').then((response) => {
    ...
    <>
      <div className={`d-flex flex-column ${style.mainHome}`}>
        <div className="d-flex justify-content-between bg-primary border border-1 border-dark">
          ...
          <button className={`btn btn-primary text-end text-white ${style.logoutBtn}`} onClick={logOut}>
        ...
        <div className="d-flex justify-content-between align-items-center px-1 p-1">
          <div className="fs-5">
            Hello, <span className="fw-bold text-black">{orgName}</span>!
          </div>
          <div>
            <div>
            ...
            <button className="btn btn-success fw-bold w-100" onClick={() => { setShowLayer(true); }}>
              <PaidIcon />
              {' '}
              Recharge
            </button>
