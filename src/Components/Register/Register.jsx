...
function Register() {
  ...
  // function takes the input and set it to the organization object (state).
  function handleOrg(event) {
    const org = { ...orgainzation };
    org[event.target.name] = event.target.value;
    setOrgainzation(org);
    // console.log(orgainzation);
  }
  // .message("name is required")  .message("email is required")
  // .message("phone number must be graeter than 10 numbers.")
  // .message("address is required")
  // .message("postal code is required and must be less than 5 numbers")
  ...
  async function submitForm(e) {
    ...
    setIsFetching(true);
    setIsSuccess(true);
    const formvalidationResult = formValidation();
    if (formvalidationResult.error) {
      ...
      try {
        const res = await publicRequest.post('register', orgainzation);
        console.log(res);
        setIsFetching(false);
          ...
          <div className={`col-md-5 text-start ${style.registerForm} bg-white  bg-opacity-75 fw-bold border border-primary border-5`}>
            ...
            <form className="m-lg-4 m-md-2 m-0">
              ...
              <div className="validationErrors  ">
                {formValidationErrors.map((err, index) => (
                  <Alert key={index} severity="error" className="my-2 p-2">
                    {' '}
                    {err.message}
                    {' '}
                  </Alert>
